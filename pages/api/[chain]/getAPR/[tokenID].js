...
async function calculateAmount0Amount1(resp) {
  ...
  return [+tokenToSignificant(amount0, decimals0), +tokenToSignificant(amount1, decimals1)]
}
...
async function calculatePrincipal(resp, ethPriceUSD, amount0, amount1) {
  ...
  return [amount0 * token0InUSD + amount1 + token1InUSD, token0InUSD, token1InUSD];
}
...
export default async function handler( req, res) {
  ...
  const [principalInUsd, token0InUSD, token1InUSD] = await calculatePrincipal(resp, ethPriceUSD, amount0, amount1)
